...
bool readFileToMatrix (const std::string &filePath, Matrix &mat)
{
	...
	long int expected_size = mat.get_rows() * mat.get_cols() * sizeof(float);
	...
	int total_elements = mat.get_rows() * mat.get_cols();
	float* temp_data = new float[total_elements];

	is.read(reinterpret_cast<char*>(temp_data), expected_size);
	if (!is) {
		delete[] temp_data;
		is.close();
		return false;
